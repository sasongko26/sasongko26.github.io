...
Konsep pertama: _**root as main directory**_. `Root` adalah direktori utama dari sistem _linux_ dan _BSD_ sekeluarga. Direktori ini dilambangkan dengan `/`. Semua direktori berada di bawah `/`. Contoh: `/bin` addalah direktori `bin` yang tepat berada di bawah `/`. `/usr/bin` adalah direktori `bin` yang di bawah `usr` dan direktori `usr` ini di bawah _main directory_ `/`.
